...
public class SubscribesDB extends Data {
    public void addGroupId(long guildId, Integer groupId){
        try (var statement = database.getConnection().createStatement()) {
            var sql = String.format("INSERT INTO `subs` (guild, groupId) VALUES (%d, %d);", guildId, groupId);
    ...
    public HashSet<Integer> getAllSubsByGuildId(long guildId){
        var result = new HashSet<Integer>();
        try (var statement = database.getConnection().createStatement()) {
            var sql = String.format("SELECT * FROM subs WHERE guild=%d;", guildId);
            var resultSet = statement.executeQuery(sql);
            while (resultSet.next())
                result.add(resultSet.getInt("groupId"));
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return result;
